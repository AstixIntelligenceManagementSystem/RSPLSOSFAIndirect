...
public class Delivery_Details_Activity extends Activity implements  DatePickerDialog.OnDateSetListener,TimePickerDialog.OnTimeSetListener{
	private TextView PaymentStageTextView;
	private TextView paymentModeTextviewNew;
	private TextView creditdaysTextboxNew;
	private TextView CreditlimitTextboxNew;
	private TextView percentageTextviewNew;
	private TextView paymentstagetextviewNew;
	TextView CreditDaysTextbox;
	TextView PaymentModeTextView;
	TextView Date;
	TextView SalesQuoteTypeSpinner;
	TextView ValFrom;
	TextView ValTo;
	TextView SalesQuoteType;
	TextView ValidityFrom;
	TextView PaymentTerms;
	TextView headerstring;
	RelativeLayout DeliveryDetailsHeader;
	private RelativeLayout delivey_details_parent;
	RelativeLayout PaymentDetailsHeader;
	private RelativeLayout Payment_Details_Parent;
	private CheckBox arrowOfDeleveryInfo;
	private CheckBox arrowOfPaymentDetails;
	private ImageView img_back;
	private ImageView CalenderValTo;
	private ImageView DeliveryTimeImg_From;
	private ImageView DeliveryTimeImg_To;
	private ImageView DoNotDeliverTimeImg_From;
	private ImageView DoNotDeliverTimeImg_To;
	private TextView RequiredDeliveryDate;
	private TextView Delivery_Time_From;
	private TextView biltocustomernameSpinner;
	private TextView Billing_AddressText;
	private TextView Delivery_Time_To;
	private TextView DoNotDeliverTime_From;
	private TextView DoNotDeliverTime_To;
	LinearLayout ll_data;
	private LinearLayout parentOfAdvanceBeforeDeliveryPayMentMode;
	private LinearLayout parentOfOnDeliveryPayMentMode;
	private LinearLayout parentOfCreditPayMentMode;
	LinearLayout parentOfCheckBox;
	private String storeID;
	private String OrderPDAID;
	private String imei;
	private String date;
	private String pickerDate;
	private String SN;
	private boolean DeliveryTimeImg_From_boolean=false;
	private boolean DeliveryTimeImg_To_boolean=false;
	private boolean DoNotDeliverTimeImg_From_boolean=false;
	private boolean DoNotDeliverTimeImg_To_boolean=false;
	...
	private TextView Delivery_Location_Spinner;
	 private DBAdapterKenya helperDb;
	 private ArrayAdapter<String> adapterDeliveryAddress;
	private ArrayAdapter<String> adapterBillingAddress;
	 private ArrayList<LinkedHashMap<String, String>> arrgetStoreBillToAddressAndDiliverToAddress;
	 private LinkedHashMap<String, String> hmapStoreBillTogAddresse;
	  private LinkedHashMap<String, String> hmapStoreShippToAddress;
	  private LinkedHashMap<String, String> hmapStoreAddress;
	  private String[]DeliverAddresstList;
	private String[] BillingAdressList;
	 private LinkedHashMap<String, String> hmapAllValuesOfPaymentMode;
	private String globalValueOfPaymentStageCheck="0"+"_"+"0"+"_"+"0";
	 private LinkedHashMap<String,String> hmapZoneDisplayMstr;
	private CheckBox chBoxView;
	private CheckBox AdvanceBeforeDeliveryCheckBoxNew;
	private CheckBox OnDeliveryCheckBoxNew;
	private CheckBox CreditCheckBoxNew;
	private ListView listDeliveryLocation;
	private ListView listBillingAdress;
	private TextView Delivery_Location_Address;
	private EditText percentageOfAdvanceBeforeDelivery;
	private EditText percentageOfOnDelivery;
	private EditText creditDaysOfOnDelivery;
	private EditText PercentageOfCredit;
	private EditText creditDaysOfCredit;
	private EditText creditLimitOfCredit;
	private LinearLayout MainlayoutParentOfWholePage;
	  private String fullStringOfDelivery="0";
	...
	private void fillAllDataOfDeliverySectionToView() {
					 ...
					 for(Map.Entry<String, String> entry:hmapStoreBillTogAddresse.entrySet())
						{
						 ...
						 String keyHashmap= entry.getKey().trim();
							String ValueHashMap= entry.getValue().trim();
	...
	private void initializeDropdown() {
					 ...
					 EditText inputSearch=	 (EditText) convertView.findViewById(R.id.inputSearch);
					  ...
					  int index=0;
						if(hmapStoreShippToAddress!=null)
				    	{
							...
							LinkedHashMap<String, String> map = new LinkedHashMap<>(hmapStoreShippToAddress);
						 ...
						 adapterDeliveryAddress = new ArrayAdapter<>(Delivery_Details_Activity.this, R.layout.list_item, R.id.product_name, DeliverAddresstList);
					 ...
					 EditText inputSearch=	 (EditText) convertView.findViewById(R.id.inputSearch);
					  ...
					  int index=0;
						if(hmapStoreBillTogAddresse!=null)
				    	{
							...
							LinkedHashMap<String, String> map = new LinkedHashMap<>(hmapStoreBillTogAddresse);
							 Set set2 = map.entrySet();
					            Iterator iterator = set2.iterator();
					            while(iterator.hasNext()) {
					            	 Map.Entry me2 = (Map.Entry)iterator.next();
						 ...
						 adapterBillingAddress = new ArrayAdapter<>(Delivery_Details_Activity.this, R.layout.list_item, R.id.product_name, BillingAdressList);
	 ...
	 private boolean validatecheckboxofAdvance() {
			...
			int count = parentOfAdvanceBeforeDeliveryPayMentMode.getChildCount();
			 ...
			 for (int ui=0;ui<count;ui++) 
			 {
				 View ch = parentOfAdvanceBeforeDeliveryPayMentMode.getChildAt(ui);
		            ...
		            {
		            	if(((CompoundButton) ch).isChecked())
						{
		            		ckeckbox=true;
	 ...
	 private boolean validatecheckboxofDelivery() {
		...
		private boolean validatecheckboxofCredit() {
		...
		private void savingDataOfDeliverySectionTodatabase() {
	...
	public boolean validatePaymentStageID()
	 {
		 ...
		 if(globalValueOfPaymentStageCheck.equals("0"+"_"+"0"+"_"+"0")){
			...
			*/else if(!validatecheckboxofAdvance() && globalValueOfPaymentStageCheck.split(Pattern.quote("_"))[0].equals("1")){
				...
				CreditlimitTextboxNew.setError(getResources().getString(R.string.CreditDayZero));
				returnResult= false;
			}
		 return returnResult;
	 }
	
	private void initializeAllViewOfPaymentSection() {
		...
		removeErrorMsgwhenclickOnedittext();
		 ...
		 if(helperDb.checkCountIntblStoreSalesOrderPaymentDetails(storeID,OrderPDAID)==1){
			 ...
			 fillValuesInPaymentSection();
		 }
		
		

	}
	private void enablePaymntMdOfAdvanceBeforeDelivery() {
		int count = parentOfAdvanceBeforeDeliveryPayMentMode.getChildCount();
	...
	private void fillValuesInPaymentSection() {
		//String all	helperDb.fngettblNewStoreSalesQuotePaymentDetails(AddNewStore_DynamicSectionWise.selStoreID)
        String allValuesOfPaymentStageID = helperDb.fngettblStoreSalesOrderPaymentDetails(storeID, OrderPDAID);
			...
			pymntStagIDofAdvn= allValuesOfPaymentStageID.split(Pattern.quote("~"))[0].trim();
			
		String percentageOfAdvn= allValuesOfPaymentStageID.split(Pattern.quote("~"))[1].trim();
			String paymentModeIdOfADVNC= allValuesOfPaymentStageID.split(Pattern.quote("~"))[4].trim();
	     ...
	     pymntStagIDofDelivery= allValuesOfPaymentStageID.split(Pattern.quote("~"))[0].trim();
	    		String percentageofDelivery= allValuesOfPaymentStageID.split(Pattern.quote("~"))[1].trim();
	    String creditDysofDelivery= allValuesOfPaymentStageID.split(Pattern.quote("~"))[2].trim();
	    String pymntmodeIDofDElivery= allValuesOfPaymentStageID.split(Pattern.quote("~"))[4].trim();
	    ...
	    if(pymntmodeIDofDElivery.contains("|")){
	      ...
	      if(allValuesOfPaymentStageID.split(Pattern.quote("~"))[0].equals("3")){
	    	  ...
	    	  pymntStagIDofcredit= allValuesOfPaymentStageID.split(Pattern.quote("~"))[0].trim();
	    	  String percentageofcredit= allValuesOfPaymentStageID.split(Pattern.quote("~"))[1].trim();
	    	  String creditDysofcredit = allValuesOfPaymentStageID.split(Pattern.quote("~"))[2].trim();
	    	  String creditLimitofcredit= allValuesOfPaymentStageID.split(Pattern.quote("~"))[3].trim();
	    	  String pymntmodeIDofcredit = allValuesOfPaymentStageID.split(Pattern.quote("~"))[4].trim();
	...
	private void enablePaymntMdOfOnDelivery() {
		int count = parentOfOnDeliveryPayMentMode.getChildCount();
	...
	private void enablePaymntMdOfCredit() {
		int count = parentOfCreditPayMentMode.getChildCount();
	...
	private void removeErrorMsgwhenclickOnedittext() {
	...
	private void disableAndUncheckPaymntMdOfCredit() {
		int count = parentOfCreditPayMentMode.getChildCount();
	...
	private void disableAndUncheckPaymntMdOfAdvanceBeforeDelivery() {
		int count = parentOfAdvanceBeforeDeliveryPayMentMode.getChildCount();
	...
	private void disableAndUncheckPaymntMdOfOnDelivery() {
		int count = parentOfOnDeliveryPayMentMode.getChildCount();
	...
	private void whenPaymentStageClickNew() {
	...
	private OnClickListener getOnClickDoSomething(final CheckBox button) {
	...
	private void checkBoxCreationwhenPageLoading(String paymentStageID) {
		...
		{
        	 int checkBoxDescId=Integer.parseInt(entry.getKey());
			 String checkBoxDesc= entry.getValue().trim();
	...
	private void arrowHeaderSelection() {
	...
	private boolean validateDelivery_Location_Spinner()
	{
		...
		if(helperDb.checkIfCurrentStoreIsNewInOrderBill(storeID)==0)
		{
			if(Delivery_Location_Spinner.getText().toString().trim().equals("Select"))
			{
				alertError(getResources().getString(R.string.DelvryLoc));
			}
			else
			{
